...
final class AppState {
    ...
    var selectedTheme: AppTheme = AppTheme(rawValue: UserDefaults.standard.string(forKey: "selectedTheme") ?? "") ?? .black {
        didSet {
    ...
    func initialize() async {
        ...
        // Migrate legacy Grove JSON sessions to CLI-compatible jsonl so they can
        ...
        let legacySummaries = allSessionSummaries.filter { $0.origin == .legacyGrove }
    ...
    func forkFromHere(messageId: UUID, in window: WindowState) async {
        ...
        // `claude --resume` picker, matching how Grove-spawned sessions are exposed.
    ...
    /// One-time migration: converts legacy Grove JSON sessions to CLI-compatible jsonl files.
