...
/// Manages an ed25519 SSH key dedicated to Grove for GitHub access.
///
...
actor SSHKeyManager {
    ...
    /// Grove key, but only if such an entry is not already present.
    func configureSSHConfig() async throws {
        ...
        logger.info("Added Grove SSH config entry.")
    }
