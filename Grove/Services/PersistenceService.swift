...
actor PersistenceService {
    ...
    func saveSession(_ session: ChatSession, persistTitle: Bool = false) async throws {
        ...
        case .cliBacked:
            // CLI owns the message log (jsonl). We only persist the Grove-only
    ...
    func loadLegacySessions(for projectId: UUID) -> [ChatSession.Summary] {
        ...
        return files
            ...
            .compactMap { url -> ChatSession.Summary? in
                ...
                // Files saved before this change may decode as `.legacyGrove`
                // already (default), but be defensive.
                if summary.origin != .legacyGrove {
    ...
    func deleteSession(projectId: UUID, sessionId: String, origin: SessionOrigin, cwd: String?) async throws {
        ...
        case .cliBacked:
            ...
            // Pre-cli-sync builds wrote a Grove-side json with the same sid. If
    ...
    /// - `.cliBacked` → CLI jsonl
    /// - `.legacyGrove` → Grove's per-project json
