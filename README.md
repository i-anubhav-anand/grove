...
> **Status:** preview. Ships frequently; not yet a stable `1.0`. See [Known limitations](#known-limitations).

![Grove 0.5.0](docs/screenshot.png)
...
- **Readable agent runs.** Streaming responses, compact one-line tool rows, file-edit pills, and clear thinking/output separation — not a wall of terminal text.
...
- **Command palette** (`⌘K`) — jump to any workspace, session, project, or action.
- **MCP-aware** — renders MCP tools and their output.

## Install

### Option A — Run from source (Xcode)

Requires macOS 15+ and Xcode with a Swift 6.2 toolchain.

```bash
git clone https://github.com/i-anubhav-anand/grove.git
cd grove
...
### Option B — Build a standalone `.app` (no Xcode needed to run)

```bash
git clone https://github.com/i-anubhav-anand/grove.git
cd grove

xcodebuild -project Grove.xcodeproj -scheme Grove -configuration Release \
  -derivedDataPath build \
  CODE_SIGN_IDENTITY="-" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO build

cp -R build/Build/Products/Release/Grove.app /Applications/

# Remove the quarantine flag so macOS doesn't block the ad-hoc signed binary
xattr -dr com.apple.quarantine /Applications/Grove.app
```

Then launch from Spotlight or the Dock like any other app.

> **Sharing with someone else:** zip the `.app` (`ditto -c -k --keepParent Grove.app Grove.zip`), send it, and have them run the same `xattr` line after unzipping — or right-click → Open the first time.

## Build & test

```bash
# Packages only (no Xcode)
cd Packages && swift build && swift test

# Full app
xcodebuild -project Grove.xcodeproj -scheme Grove \
