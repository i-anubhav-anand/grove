...
/// Rewrites Claude Code session jsonl files so that Grove-spawned sessions
...
/// mode (`-p`), which is how Grove spawns it. The picker filters those
/// sessions out, so without this normalization Grove's history is invisible
